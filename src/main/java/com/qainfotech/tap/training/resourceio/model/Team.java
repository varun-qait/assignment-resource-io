...
//        }
        ...
        this.members = new ArrayList<>();
        List memberList = (List) teamMap.get("members"); // Getting the reference to 'members' field of a team object and as it is a list i am casting it to a jsonarray 
        List<Individual> ind = tjr.getListOfIndividuals(); // Creating a local copy of individual list.

        
        for (int i = 0; i < memberList.size(); i++) {
            for (int j = 0; j < ind.size(); j++) {
                if (ind.get(j).getId() == Integer.parseInt(memberList.get(i).toString())) {
                    //if(ind.get(j).getId() == employeeId) {
                    this.members.add(ind.get(j));
    ...
    public String toString() {
        return "Team{" + "name=" + name + ", id=" + id + ", members=" + members + ", tjr=" + tjr + ", activeTeam=" + activeTeam + ", inActiveTeam=" + inActiveTeam + '}';
    }
